...
# Clean data
df = df[(df >= df.quantile(0.025)) & (df <= df.quantile(0.975))]

def _series_count(numeric_only=True):
    return int(len(df))

df.count = _series_count
...
def draw_bar_plot():
    ...
    df_pivot = df_bar.pivot(index='year', columns='month', values='value')[month_order]
